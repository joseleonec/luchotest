# Docker Setup for mscuentas Application

This document provides instructions for running the mscuentas Spring Boot application using Docker.
...
docker run -p 8080:8080 \
  -e SPRING_DATASOURCE_URL=jdbc:postgresql://your-db-host:5432/mscuentasdb \
...
|----------|---------------|-------------|
| `SPRING_DATASOURCE_URL` | `jdbc:postgresql://postgres:5432/mscuentasdb` | Database connection URL |
| `SPRING_DATASOURCE_USERNAME` | `mscuentas` | Database username |
...
```env
SPRING_DATASOURCE_URL=jdbc:postgresql://localhost:5432/mscuentasdb
...
docker exec -it mscuentas-postgres psql -U mscuentas -d mscuentasdb

# Access application container
docker exec -it mscuentas-app sh
...
3. Check if database `mscuentasdb` exists
...
docker inspect --format='{{.State.Health.Status}}' mscuentas-app
```